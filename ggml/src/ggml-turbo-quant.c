...
void quantize_row_turbo1_ref(const float * GGML_RESTRICT x, block_turbo1 * GGML_RESTRICT y, int64_t k) {
    turbo_init_rotation();

    assert(k % QK_TURBO1 == 0);
    const int nb = k / QK_TURBO1;
    const int d  = QK_TURBO1;

    for (int block = 0; block < nb; block++) {
        const float * src = x + block * d;

        /* Step 1: Extract norm + normalize */
        float norm_sq = 0.0f;
        for (int i = 0; i < d; i++) norm_sq += src[i] * src[i];
        float norm = sqrtf(norm_sq);
        float normalized[TURBO_D];
        ...
        /* Step 3: 1-bit sign + norm-correcting scale (recon norm = sqrt(d) for a sign code) */
        const float inv_sqrt_d = 1.0f / sqrtf((float)d);

        for (int i = 0; i < d / 8; i++) y[block].qs[i] = 0;
        for (int i = 0; i < d; i++) {
            if (rotated[i] < 0.0f) y[block].qs[i >> 3] |= (uint8_t)(1u << (i & 7));
        }
        y[block].d = GGML_FP32_TO_FP16(norm * inv_sqrt_d);
    }
}

void dequantize_row_turbo1(const block_turbo1 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    turbo_init_rotation();
    ...
    for (int block = 0; block < nb; block++) {
        float scale = GGML_FP16_TO_FP32(x[block].d);

        /* Reconstruct in rotated space: sign * scale */
        float rotated_recon[TURBO_D];
        for (int i = 0; i < d; i++) {
            uint8_t bit = (x[block].qs[i >> 3] >> (i & 7)) & 1u;
            rotated_recon[i] = bit ? -scale : scale;
        }
...
size_t quantize_turbo1(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst,
                       int64_t nrows, int64_t n_per_row, const float * imatrix) {
    GGML_UNUSED(imatrix);
    assert(n_per_row % QK_TURBO1 == 0);

    size_t row_size = (n_per_row / QK_TURBO1) * sizeof(block_turbo1);
    for (int64_t row = 0; row < nrows; row++) {
        quantize_row_turbo1_ref(
            src + row * n_per_row,
            (block_turbo1 *)((char *)dst + row * row_size),
            n_per_row
        );
    }
    return nrows * row_size;
...
// turbo1_nsn CPU ref: STUB (o=0, no centering — the per-chunk per-head mean is GPU-only). Not used
// for KV decode (fattn is). With o=0: s2=1, equivalent to turbo1 with s1=norm/sqrt(128).
void quantize_row_turbo1_nsn_ref(const float * GGML_RESTRICT x, block_turbo1_nsn * GGML_RESTRICT y, int64_t k) {
    turbo_init_rotation();
    assert(k % QK_TURBO1_NSN == 0);
    const int nb = k / QK_TURBO1_NSN;
    const int d  = QK_TURBO1_NSN;
    const float inv_sqrt_d = 1.0f / sqrtf((float)d);
    for (int block = 0; block < nb; block++) {
        const float * src = x + block * d;
        float norm_sq = 0.0f;
        ...
        float norm = sqrtf(norm_sq);
        float normalized[TURBO_D];
        if (norm > 1e-10f) { const float inv = 1.0f/norm; for (int i=0;i<d;i++) normalized[i]=src[i]*inv; }
        else memset(normalized, 0, d*sizeof(float));
        float rotated[TURBO_D];
        matvec(turbo_rotation, normalized, rotated, d);
        for (int i = 0; i < d/8; i++) y[block].qs[i] = 0;
        for (int i = 0; i < d; i++) if (rotated[i] < 0.0f) y[block].qs[i>>3] |= (uint8_t)(1u<<(i&7));
        y[block].s1 = GGML_FP32_TO_FP16(norm * inv_sqrt_d);
        y[block].s2 = GGML_FP32_TO_FP16(1.0f);
    }
}

void dequantize_row_turbo1_nsn(const block_turbo1_nsn * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    turbo_init_rotation();
    assert(k % QK_TURBO1_NSN == 0);
    const int nb = k / QK_TURBO1_NSN;
    const int d  = QK_TURBO1_NSN;
    const float sqrt_d = sqrtf((float)d);
    const float inv_sqrt_d = 1.0f / sqrt_d;
    for (int block = 0; block < nb; block++) {
        float s1 = GGML_FP16_TO_FP32(x[block].s1);
        float s2 = GGML_FP16_TO_FP32(x[block].s2);
        float a = s1 * s2 * sqrt_d * inv_sqrt_d;   // = s1*s2 (o=0 stub)
        float rotated_recon[TURBO_D];
        for (int i = 0; i < d; i++) {
            uint8_t bit = (x[block].qs[i>>3] >> (i&7)) & 1u;
            rotated_recon[i] = bit ? -a : a;
        }
        float * dst = y + block * d;
        matvec(turbo_rotation_t, rotated_recon, dst, d);
    }
...
size_t quantize_turbo1_nsn(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst,
                           int64_t nrows, int64_t n_per_row, const float * imatrix) {
    GGML_UNUSED(imatrix);
    assert(n_per_row % QK_TURBO1_NSN == 0);
    size_t row_size = (n_per_row / QK_TURBO1_NSN) * sizeof(block_turbo1_nsn);
    for (int64_t row = 0; row < nrows; row++) {
        quantize_row_turbo1_nsn_ref(src + row * n_per_row,
            (block_turbo1_nsn *)((char *)dst + row * row_size), n_per_row);
    }
    return nrows * row_size;
}

// turbo1_cq CPU ref: STUB (the 256x8 codebook is CUDA-only; CPU path not used for KV decode).
void quantize_row_turbo1_cq_ref(const float * GGML_RESTRICT x, block_turbo1_cq * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_TURBO1_CQ == 0);
    const int nb = k / QK_TURBO1_CQ;
    for (int b = 0; b < nb; b++) {
        const float * src = x + b * QK_TURBO1_CQ;
        float ns = 0.0f; for (int i = 0; i < QK_TURBO1_CQ; i++) ns += src[i]*src[i];
        y[b].d = GGML_FP32_TO_FP16(sqrtf(ns) * 0.08838834764831845f);
        for (int i = 0; i < QK_TURBO1_CQ/8; i++) y[b].qs[i] = 0;
    }
}
void dequantize_row_turbo1_cq(const block_turbo1_cq * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_TURBO1_CQ == 0);
    memset(y, 0, (size_t)k * sizeof(float)); // stub; real decode is the CUDA codebook lookup
    (void)x;
}
size_t quantize_turbo1_cq(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst,
                          int64_t nrows, int64_t n_per_row, const float * imatrix) {
    GGML_UNUSED(imatrix);
    assert(n_per_row % QK_TURBO1_CQ == 0);
    size_t row_size = (n_per_row / QK_TURBO1_CQ) * sizeof(block_turbo1_cq);
    for (int64_t row = 0; row < nrows; row++) {
        quantize_row_turbo1_cq_ref(src + row * n_per_row,
            (block_turbo1_cq *)((char *)dst + row * row_size), n_per_row);
    }
    return nrows * row_size;
}
